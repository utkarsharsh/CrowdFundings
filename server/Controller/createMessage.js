...
const createMessageAndUpdateCharityMaterial = async (req, res) => {
  try {
    const { address, description,charityId } = req.body;
    console.log("0")
    if(!description || !charityId){
        res.status(400).json({
            success: false,
    ...
    console.log("2")
    const data = await CharityMaterial.findByIdAndUpdate(charityId,{$push:{messages:message._id}},{new:true}).populate("messages")
